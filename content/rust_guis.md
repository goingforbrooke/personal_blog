...
Not so much a GUI crate as a super-wrapper of sorts, Tauri offers a ["platform webview"](https://youtu.be/XjbVnwBtVEk?si=e5v-cyhWabnn4h23&t=356) reminiscent of [Electron](https://www.electronjs.org) that [consumes less system resources](https://www.youtube.com/watch?v=UxTJeEbZX-0&t=233s).

> Electron's the webpage-in-a-box behind VSCode, Slack, Discord, GitHub Desktop, Atom, and Skype.
...
> "Come for the Rust, but stay for the JS. It's a terrible claim, but basically, what we're trying to do with the API is lower the barrier of entry for people who want the safety guarantees of Rust and the very strict approach to access of the system assets that we can offer from Tauri. It's about good configuration and a good API." -- Denjell

{{ youtube(id="BhmXTi0X7Kg?si=C5cH9eI2kc08qeK1" start="38:20") }}

The Sveltekit intro tracks with the unparalleled [docs](https://tauri.app/v1/guides/) that [Lorenzo Lewis](https://twitter.com/lorenzoilewis) is making [even better](https://beta.tauri.app). He and the other Tauri folks had a warm presence at [RustConf](https://rustconf.com) this year and their partner company [Crab Nebula](https://crabnebula.dev/blog/tauri-partnership) continues to inject love into the developer community.

Developer prioritization culiminates in an inspring CI ecosystem with deployment capabilities approaching that of [goreleaser](https://github.com/goreleaser/goreleaser). My [CI piplelines](@/action_universal.md) borrow heavily from Tauri's example.
...
My biggest issue with Tauri was how much it slowed filesytem access. Compared to [egui](#egui), It took significantly longer to count every file on my machine, probably because of a mechanism that checks each access attempt against a [whitelist](https://tauri.app/v1/api/js/fs/). It's an ambitious security standard that I'd like to see more projects aspire to, but I really want my Rust code to run unimpeded.

When I looked for ways to expand the project, I quickly got lost in the world of [Vite](https://vitejs.dev) and other [WASM](https://webassembly.org)-related stuff. These helpers seem to be salient knowledge for dealing with compilation errors, but I was deterred by the prospect of developing esoteric GUI knowlege for a desktop app.

By the end of my time with it, I felt that Tauri was focused more on being powered by Rust than being *for* Rust. The CI/CD, documentation, and explicit security were wonderful, but those conveniences weren't enough to justify mixing NodeJS stuff into my project. Inspired by [Iced](#iced), I learned [Elm](https://elm-lang.org), but found that fun little detour to be a weird little Haskell-world unto itself.
...
The default look won't win any awards, but built-in graphs give us JavaScript-like visualization capabilities that other GUI libraries are only beginning to add. A few [tweaks](https://github.com/emilk/egui#can-i-customize-the-look-of-egui) to the global theme look pretty sweet pretty fast, but it's tedious to [style elements in groups](https://github.com/emilk/egui/issues/3284).

> Best of all, it has a dark theme! 🤯

After watching the project's incredible growth over the last year, I'm confident that what doesn't already exist won't be that far off. A good bit of project churn has made the Rust ecosystem feel more chaotic than it is. The benevolent care of [emilk](https://twitter.com/ernerfeldt) really shone through during my evaluation phase as other libraries fell into disuse.

What egui doesn't have, I'm certain I could add myself. Beyond the API, its internals too are incredibly simple. Take, for instance, the code necessary for the [hello world example](https://github.com/emilk/egui/blob/0.25.0/examples/hello_world/src/main.rs).

{{ image(src="egui_hello_world.png",
         alt="Screenshot of a simple application with a slider and a picture of Ferris the crab.",
         style="border-radius: 8px;") }}

I left out the boilerplate because the meat of the GUI lives in `update()`. This [immediate approach](https://nikolish.in/gs-with-iced-1) calls `update()` [many times a second](https://github.com/emilk/egui/issues/1109#issuecomment-1013742990), completely redrawing the screen each time 🤣.
...
Those redraws add some CPU strain, but that's not a big deal if the GUI has its own thread. Mobile targets stand to suffer the most due to the impact on battery life, but that could still happen because optimzation, like [animations](https://github.com/lucasmerlin/hello_egui#experimental-crates), hasn't been explored yet.
...
The benefit of barbarous rendering is a crisp response that few have touched since the programs of yore. Clicks register almost presciently, building user trust intuitively. That is, if my user can ignore the mounting suspicion that they've just downloaded a trojan.

Here's how [my project's](https://github.com/goingforbrooke/folsum/tree/main) looking with egui. The "Summarize" button doesn't work in WASM builds because I haven't dummy files (yet).

{{ embedpage(url="https://www.goingforbrooke.com/folsum/") }}

Layout capabilities are the biggest deficiency of the immediate approach because it intrinsically forbids the concept of a layout as we know it. Neatly proportional panes and multiple "views" aren't in the cards when you're tossing in elements from the top left to bottom right.

Egui happily deployed to Windows and MacOS with no code or dependency changes. In a strange turn of events, it even [fits inside](https://tauri.app/blog/2022/09/19/tauri-egui-0-1/) [Tauri](#tauri).
...
[Xilem](https://github.com/emilk/egui/blob/master/README.md) looks promising, but it's [more idea](https://www.xilem.dev) than library. It aims to becomes Rust's definitive UI framework by incorporating lessons from SwiftUI and [Druid](https://github.com/linebender/druid), a popular create that was discontinued in favor of Xilem. I'm excited to watch [Raph Levien’s](https://mastodon.online/@raph) [vision](https://raphlinus.github.io/rust/gui/2022/05/07/ui-architecture.html) mature into an incredible developer experience.
...
[Slint](https://github.com/slint-ui/slint) had a nice look and seemed to [cater to embedded systems](https://madewithslint.com), which I hoped would translate to portability. The creators wanted to move on from Qt to a mouse-driven UI creation that could then be wired up to code.
...
[egui](#egui) has my heart at the moment, but I'm eager to try [Dioxus](#honorable-mention-dioxus). [Xilem](#honorable-mention-xilem) might eventually come to save us all, but as of now, these are the most viable bridges to mouse-land. 🐁

# Additional Material

The list of Rust GUI projects on [areweguiyet.com](https://areweguiyet.com) is regularly updated with new and deceased repos. The merits of the most popular options are regularly updated on LogRocket's [blog](https://blog.logrocket.com/state-of-rust-gui-libraries/).

Igor Loskutov's [50 shades of Rust](https://monadical.com/posts/shades-of-rust-gui-library-list.html#18-Makepad) offers code samples and thorough commentary on an incredible breadth of 50 libraries.

The [State of Rust GUIs](https://dev.to/davidedelpapa/rust-gui-introduction-a-k-a-the-state-of-rust-gui-libraries-as-of-january-2021-40gl) post offers the most code rich overview. The code samples are getting out of date, but they convey the ethos of each library.

The [performance comparision of Tauri, Iced, and egui](http://lukaskalbertodt.github.io/2023/02/03/tauri-iced-egui-performance-comparison.html) paints an intriguing picture of each approach.
